# Plantilla: Journey Map — Experiencia de Frustración del Cliente

## Nombre del estudiante: _______________________________________

## Fecha: _______________________________________

---

## Instrucciones de uso

Esta plantilla te ayuda a mapear la experiencia de un cliente de RetailNova que tuvo una experiencia frustrada. Usa como base el caso de Laura Mosquera (el cliente que se fue a Zara), pero puedes adaptarlo a otro testimonio del dossier si lo prefieres. El objetivo es identificar puntos de contacto (touchpoints), emociones y datos faltantes que causaron la frustración.

**Tiempo sugerido:** 15 minutos para esta sección.

---

## Paso 1: Define el perfil del cliente

| Campo | Respuesta |
|---|---|
| **Nombre del cliente** | (Puedes usar el nombre del dossier o crear uno representativo basado en los testimonios) |
| **Edad / perfil estimado** | _ |
| **Canal por el que conoció a RetailNova** | Instagram / WhatsApp / Recomendación / Pasó frente a la tienda / Otro |
| **Frecuencia de compra antes del incidente** | Primera vez / Ocasional / Regular / Frecuente |
| **Motivo de la visita este día** | _ |

**Justificación del perfil elegido** (por qué este perfil es representativo de los clientes de RetailNova):

> Ejemplo: "Laura es una mujer joven de ~28 años, activa en redes sociales, que descubrió RetailNova por Instagram y planeó (planned) una visita específica a la tienda. Representa el perfil de cliente que RetailNova quiere atraer: digitalmente activo, con intención de compra, pero que se frustra cuando la experiencia no es consistente con lo que ve en línea."

---

## Paso 2: Define el problema que disparó (trigger) la frustración

| Campo | Respuesta |
|---|---|
| **Problema principal del journey** | _ |
| **Expectativa del cliente antes de la experiencia** | _ |
| **Qué esperaba encontrar vs. qué encontró** | _ |
| **Evidencia del dossier que respalda este problema** | _ |

---

## Paso 3: Mapea las etapas del journey

Completa la tabla con cada etapa del journey. Un journey de frustración debe tener mínimo 4 etapas y máximo 8. Cada etapa debe incluir acción, dolor, emoción, punto de contacto (touchpoint) y dato faltante.

---

### Etapa 1: [NOMBRE DE LA ETAPA]

| Elemento | Descripción |
|---|---|
| **Etapa** | 1. Descubrimiento / 2. Consideración / 3. Preparación / 4. Viaje / 5. Llegada a tienda / 6. Interacción en tienda / 7. Frustración / 8. Deserción / 9. Compra alternativa / Otro |
| **Acción del cliente** | ¿Qué hizo el cliente en esta etapa? |
| **Dolencia / problema** | ¿Qué dolor sintió en esta etapa? |
| **Emoción dominante** | (Ver escala abajo) |
| **Punto de contacto (touchpoint)** | ¿Qué canal o punto de contacto usó o experimentó el cliente? |
| **Dato faltante** | ¿Qué información no estaba disponible para el cliente (o para el vendedor) en esta etapa? |
| **Nota contextual** | Cualquier detalle relevante de la etapa |

**Escala de emociones:**

| Emoción positiva (+) | Neutro (=) | Emoción negativa (-) |
|---|---|---|
| +3 Encantado | = Indiferente | -3 Furioso |
| +2 Muy satisfecho | = Sin cambio | -2 Muy frustrado |
| +1 Satisfecho | = Neutral | -1 Frustrado |
| +0 Contento | = Sin opinión | -0 Decepcionado |

---

### Etapa 2: [NOMBRE DE LA ETAPA]

| Elemento | Descripción |
|---|---|
| **Etapa** | _ |
| **Acción del cliente** | _ |
| **Dolencia / problema** | _ |
| **Emoción dominante** | _ |
| **Punto de contacto (touchpoint)** | _ |
| **Dato faltante** | _ |
| **Nota contextual** | _ |

---

### Etapa 3: [NOMBRE DE LA ETAPA]

| Elemento | Descripción |
|---|---|
| **Etapa** | _ |
| **Acción del cliente** | _ |
| **Dolencia / problema** | _ |
| **Emoción dominante** | _ |
| **Punto de contacto (touchpoint)** | _ |
| **Dato faltante** | _ |
| **Nota contextual** | _ |

---

### Etapa 4: [NOMBRE DE LA ETAPA]

| Elemento | Descripción |
|---|---|
| **Etapa** | _ |
| **Acción del cliente** | _ |
| **Dolencia / problema** | _ |
| **Emoción dominante** | _ |
| **Punto de contacto (touchpoint)** | _ |
| **Dato faltante** | _ |
| **Nota contextual** | _ |

---

### Etapa 5: [NOMBRE DE LA ETAPA]

| Elemento | Descripción |
|---|---|
| **Etapa** | _ |
| **Acción del cliente** | _ |
| **Dolencia / problema** | _ |
| **Emoción dominante** | _ |
| **Punto de contacto (touchpoint)** | _ |
| **Dato faltante** | _ |
| **Nota contextual** | _ |

---

### Etapa 6: [NOMBRE DE LA ETAPA]

| Elemento | Descripción |
|---|---|
| **Etapa** | _ |
| **Acción del cliente** | _ |
| **Dolencia / problema** | _ |
| **Emoción dominante** | _ |
| **Punto de contacto (touchpoint)** | _ |
| **Dato faltante** | _ |
| **Nota contextual** | _ |

---

### Etapa 7: [NOMBRE DE LA ETAPA] (opcional)

| Elemento | Descripción |
|---|---|
| **Etapa** | _ |
| **Acción del cliente** | _ |
| **Dolencia / problema** | _ |
| **Emoción dominante** | _ |
| **Punto de contacto (touchpoint)** | _ |
| **Dato faltante** | _ |
| **Nota contextual** | _ |

---

### Etapa 8: [NOMBRE DE LA ETAPA] (opcional)

| Elemento | Descripción |
|---|---|
| **Etapa** | _ |
| **Acción del cliente** | _ |
| **Dolencia / problema** | _ |
| **Emoción dominante** | _ |
| **Punto de contacto (touchpoint)** | _ |
| **Dato faltante** | _ |
| **Nota contextual** | _ |

---

## Paso 4: Visualización del Journey (gráfico de emociones)

Basado en las emociones que asignaste a cada etapa, indica el nivel emocional en el gráfico:

```
Emoción
  ^
+3 |                              ★★★
  |
+2 |
  |
+1 |
  |
 =0 |-----------------------------------
  |
-1 |        ★★
  |
-2 |     ★                ★★
  |
-3 |★★                         ★ ← punto más bajo (frustración máxima)
  |
  +---+---+---+---+---+---+---+---+---→ Etapas
     1   2   3   4   5   6   7   8
```

**Leyenda de emociones:**
- ★ = Emoción del cliente en cada etapa
- ~~~~ = Línea de expectativa (lo que el cliente esperaba)
- ____ = Línea de experiencia real

**Punto de máxima frustración:** Etapa _ (¿Por qué?)

**Punto de no retorno** (momento en que el cliente decide no volver): Etapa _

---

## Paso 5: Datos faltantes identificados

Enumera los datos que no estaban disponibles durante el journey y que causaron o agravaron la frustración:

| # | Dato faltante | Etapa donde ocurre | Impacto en la experiencia | ¿Cómo se resuelve? |
|---|---|---|---|---|
| 1 | _ | _ | _ | _ |
| 2 | _ | _ | _ | _ |
| 3 | _ | _ | _ | _ |
| 4 | _ | _ | _ | _ |
| 5 | _ | _ | _ | _ |

---

## Paso 6: Puntos de contacto (touchpoints) organizados por canal

| Canal | Puntos de contacto (touchpoints) identificados | Etapa(s) | ¿Está bajo control de RetailNova? (Sí/No) |
|---|---|---|---|
| Instagram | _ | _ | _ |
| WhatsApp | _ | _ | _ |
| Tienda física | _ | _ | _ |
| Teléfono/llamada | _ | _ | _ |
| Otro: _ | _ | _ | _ |

---

## Paso 7: Oportunidades de mejora priorizadas

Con base en el journey map, enumera las oportunidades de mejora más urgentes:

| Prioridad | Oportunidad | Etapa(s) afectada(s) | ¿Por qué es urgente? |
|---|---|---|---|
| 1 | _ | _ | _ |
| 2 | _ | _ | _ |
| 3 | _ | _ | _ |

---

## Nota para el estudiante

El mapa del recorrido del cliente (journey map) debe ser una herramienta accionable, no un ejercicio decorativo. Cada etapa debe ayudarte a identificar qué dato falta, qué punto de contacto (touchpoint) falla, y dónde puedes intervenir. Si tu journey no te lleva a ninguna recomendación concreta, revisa si estás siendo lo suficientemente específico.

El caso de Laura Mosquera es el más completo del dossier y tiene casi todas las etapas que necesitas. Puedes usarlo como base pero adaptar los detalles si lo consideras necesario. Lo que no puedes hacer es inventar etapas que no tengan soporte en el dossier.

---

## Ejemplo de llenado (para referencia)

**Perfil del cliente:** Laura Mosquera, mujer joven (~28 años), descubrió RetailNova por Instagram, planeó (planned) visita específica.

**Problema disparador (trigger):** No encuentra el vestido negro en su talla tras ver la promoción en Instagram.

---

### Etapa 1: Descubrimiento

| Elemento | Descripción |
|---|---|
| **Etapa** | 1. Descubrimiento |
| **Acción del cliente** | Ve publicaciones de vestidos negros en Instagram de RetailNova. Guarda la publicación. |
| **Dolencia / problema** | Ninguno en esta etapa. |
| **Emoción dominante** | +2 Satisfecha (genera ilusión) |
| **Punto de contacto (touchpoint)** | Instagram |
| **Dato faltante** | Ninguno (el cliente tiene la información que necesita para decidir) |
| **Nota contextual** | Laura ha seguido la cuenta por meses. El deseo de compra se genera por contenido de Instagram. |

---

### Etapa 2: Viaje a la tienda

| Elemento | Descripción |
|---|---|
| **Etapa** | 4. Viaje |
| **Acción del cliente** | Se transporta físicamente a la tienda centro de RetailNova. Invierte tiempo y costo de transporte. |
| **Dolencia / problema** | Incertidumbre sobre si el producto estará disponible al llegar. |
| **Emoción dominante** | +1 Contenta (expectativa alta) |
| **Punto de contacto (touchpoint)** | Ninguno durante el viaje (iniciativa del cliente) |
| **Dato faltante** | Inventario en tiempo real que le permita saber si el vestido está disponible antes de salir de su casa. |
| **Nota contextual** | Si Laura pudiera consultar disponibilidad antes de salir, evitaría el viaje frustrado. |

---

### Etapa 3: Frustración en tienda

| Elemento | Descripción |
|---|---|
| **Etapa** | 6. Interacción en tienda / 7. Frustración |
| **Acción del cliente** | Solicita el vestido negro en su talla. El vendedor revisa el área y no lo tiene. |
| **Dolencia / problema** | El producto no está disponible en la talla buscada. El vendedor no tiene cómo verificar en otra sucursal. |
| **Emoción dominante** | -2 Muy frustrada |
| **Punto de contacto (touchpoint)** | Tienda física |
| **Dato faltante** | Sistema de consulta de inventario entre sucursales en tiempo real. |
| **Nota contextual** | El vendedor actúa con la mejor información que tiene (local), pero no puede ayudar más. La frustración no es contra el vendedor sino contra la empresa. |

---

### Etapa 4: Deserción

| Elemento | Descripción |
|---|---|
| **Etapa** | 8. Deserción |
| **Acción del cliente** | Se va de la tienda sin comprar. No dice nada al salir. |
| **Dolencia / problema** | Ningún mecanismo para retenerla o compensar la frustración. |
| **Emoción dominante** | -1 Frustrada |
| **Punto de contacto (touchpoint)** | Ninguno (sale sin interactuar más) |
| **Dato faltante** | Datos de contacto del cliente para hacer seguimiento posterior. |
| **Nota contextual** | El vendedor no tiene cómo recuperarla. No hay forma de ofrecerle algo alternativo o guardar sus datos para contacto futuro. |

---

### Etapa 5: Compra alternativa

| Elemento | Descripción |
|---|---|
| **Etapa** | 9. Compra alternativa |
| **Acción del cliente** | Se va a Zara del centro comercial, donde compra el vestido que necesitaba. |
| **Dolencia / problema** | Ninguna (el cliente encuentra lo que busca en la competencia). |
| **Emoción dominante** | +1 Satisfecha (alivio al resolver su necesidad) |
| **Punto de contacto (touchpoint)** | Tienda Zara |
| **Dato faltante** | Ninguno (la competencia tiene lo que RetailNova no tuvo) |
| **Nota contextual** | El cliente priorizó la experiencia de compra sobre la marca. RetailNova pierde no por calidad de producto sino por experiencia. |

---

**Datos faltantes identificados:**
1. Inventario en tiempo real por sucursal.
2. Sistema de consulta entre sucursales para el vendedor.
3. Mecanismo de contacto del cliente (para seguimiento post-frustración).

**Oportunidades de mejora:**
1. Implementar sistema de inventario centralizado con consulta en tiempo real (prioridad 1).
2. Capacitar vendedores en uso de herramientas de consulta inter-sucursal (prioridad 2).
3. Crear registro de clientes frustrados (disappointed) para seguimiento (prioridad 3).

---

*Fin de la plantilla de Journey Map*